...
contract ProtectedVault is ERC20, IERC4626 {
    using SafeERC20 for IERC20;
    using Math for uint256;

    IERC20 private immutable ASSET; // Definition of the asset token

    uint256 virtual_offset = 12; // Virtual offset. Should be chosen depending on offset of underlying token and token's cost
    ...
    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view returns (uint256 shares) {
        shares = assets.mulDiv(totalSupply() + 10 ** virtual_offset, totalAssets() + 1, rounding);
    }

    function _convertToAssets(uint256 shares, Math.Rounding rounding) internal view returns (uint256 assets) {
        assets = shares.mulDiv(totalAssets() + 1, totalSupply() + 10 ** virtual_offset, rounding);
    }
}
